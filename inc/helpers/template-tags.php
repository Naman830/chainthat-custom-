...
function plug_posted_on() {
	...
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		...
		$time_string = '<time class="entry-date published text-dark text-decoration-none" datetime="%1$s">%2$s</time> 
		<time class="updated text-dark text-decoration-none" datetime="%3$s">%4$s</time>';
	}
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'plug' ),
 ...
 * @return void
 */
function plug_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'plug' ),
 ...
 * @return bool|string
 */
function plug_the_excerpt( $trim_character_count = 0 ) {
	$post_ID = get_the_ID();
 ...
 */
function plug_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="plug-read-more text-white mt-3 btn btn-info" href="%1$s">%2$s</a>',
